#!/usr/bin/env python3
"""
Compute the Reynolds-decomposed mixed-layer heat budget for CESM simulations.

Follows the logic of heat_budget_rd_lme.mod_zliu.m (May 2026, Z. Liu),
itself based on heat_budget_rd_lme.m (March 2015, S. Stevenson) and
the formulation of Graham et al. Climate Dynamics (2014) 43:2399-2414.

Runtime configuration is read from environment variables set by run.sh
...
# ---------------------------------------------------------------------------
DATA_FORMAT_OPTION = int(os.environ.get('DATA_FORMAT_OPTION', '3'))
BASE_DIR   = os.environ.get('BASE_DIR',  '/data4/liuzedong/archive2')
OUT_DIR    = os.environ.get('OUT_DIR',   '/lustre/home/liuzedong/fw/heatbudget')
GRID_FILE  = os.environ.get('GRID_FILE', os.path.join(
    BASE_DIR,
    'b.e10.B1850CN.T31_g37.001.rest4101', 'ocn', 'hist',
    'b.e10.B1850CN.T31_g37.001.rest4101.pop.h.5200-01.nc'))
NZ         = int(os.environ.get('NZ', '20'))
# REF_COL: 1-indexed reference column for choosing the lat/lon bands (MATLAB convention)
REF_COL    = int(os.environ.get('REF_COL', '50')) - 1   # convert to 0-indexed

_regbox_str = os.environ.get('REGBOX', '-10 10 190 240').split()
regbox = [float(x) for x in _regbox_str]   # [lat_min, lat_max, lon_min, lon_max]

# Ensemble / run / date configuration (JSON strings)
ensnames = json.loads(os.environ.get('ENSNAMES_JSON', '["CTRL"]'))
runnames = json.loads(os.environ.get('RUNNAMES_JSON',
    '[["b.e10.B1850CN.T31_g37.001.rest4101"]]'))
dates    = json.loads(os.environ.get('DATES_JSON', '["520001-530012"]'))
...
# ---------------------------------------------------------------------------
# Helper: decode the time axis using cftime → (time_vals, yr, mon)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Helper: parse a date string "YYYYMM-YYYYMM" into (yr_start, mo_start, yr_end, mo_end)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Helper: write results to a NetCDF file
# ---------------------------------------------------------------------------
...
with nc4.Dataset(GRID_FILE) as nc:
    tlat_full = _read_var(nc, 'TLAT')    # (nlat_full, nlon_full)
    tlon_full = _read_var(nc, 'TLONG')   # (nlat_full, nlon_full)

# Select the lat/lon bands that fall within the region of interest.
# REF_COL (0-indexed) is used as the reference column/row, matching MATLAB's convention.
mylat = np.where(
...
# ---------------------------------------------------------------------------
for ee, ensname in enumerate(ensnames):
    for rr, runname in enumerate(runnames[ee]):
        if not runname:
            continue
        print(f"\n=== Ensemble: {ensname}  |  Run: {runname} ===")

        for date_str in dates:
            print(f"  Processing date range: {date_str}")
            ...
            if DATA_FORMAT_OPTION == 1:
                ...
                filepath = os.path.join(BASE_DIR, runname, 'ocn', 'proc',
                                        f'{runname}.pop.h.all.{date_str}.nc')
                ...
                # Format 2: one variable per file (CESM standard monthly time series)
                data_dir = os.path.join(BASE_DIR, runname, 'ocn', 'proc', 'tseries', 'monthly')
                def _fpath(var):
                    return os.path.join(data_dir, var,
                                        f'{runname}.pop.h.{var}.{date_str}.nc')
                ...
                # Format 3: one file per month  (runname.pop.h.YYYY-MM.nc)
                data_dir = os.path.join(BASE_DIR, runname, 'ocn', 'hist')
                yr_start, mo_start, yr_end, mo_end = _parse_date_str(date_str)
                n_months = (yr_end - yr_start) * 12 + (mo_end - mo_start) + 1

                time_vals = np.zeros(n_months)
                temp = np.zeros((n_months, NZ,   len(mylat), len(mylon)))
                uvel = np.zeros_like(temp)
                vvel = np.zeros_like(temp)
                wvel = np.zeros_like(temp)
                qnet = np.zeros((n_months, len(mylat), len(mylon)))
                ...
                # Decode time for format-3 files using cftime
                # Reconstruct a dummy units string consistent with CESM noleap
                _units = 'days since 0000-01-01 00:00:00'
                decoded = cftime.num2date(time_vals, _units, calendar='noleap')
                yr  = np.array([d.year  for d in decoded], dtype=int)
                mon = np.array([d.month for d in decoded], dtype=int)
            ...
            # ----------------------------------------------------------------
            # Surface heat flux term  (vectorised, no per-time loop)
            # ----------------------------------------------------------------
            ...
            nc_name = f'{runname}_heatbudget_ml_rd_{date_str}_varyMLD.nc'
